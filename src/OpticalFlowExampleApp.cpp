...
void OpticalFlowExampleApp::setup()
{
    ...
    squareFeat.divideScreen();
    squareDiff.divideScreen();
...
cv::Mat OpticalFlowExampleApp::bsFrameDifference()
{
    ...
    cv::absdiff(input, mBGFrame, outputImg);
    cv::threshold(outputImg, outputImg, 25, 255, cv::THRESH_BINARY);
    if (keyPressed == 'd') { squareDiff.frameDifferencing(outputImg); }
    return outputImg;
}
...
void OpticalFlowExampleApp::draw()
{
    ...
    drawBGDots();
    if (keyPressed == 'f') { squareFeat.displaySquares(); }
    else if (keyPressed == 'd') { squareDiff.displaySquares(); }
}
